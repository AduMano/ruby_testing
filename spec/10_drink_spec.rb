...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:shake) { described_class.new('Shake') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(shake.type).to eq('Shake')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(shake.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:milk) { described_class.new('Milk') }
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:water) { described_class.new('Water', 1) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(water).not_to be_full
      end
    end
  end
end
