...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    ...
    subject(:new_magic_seven) { described_class.new(random_number) }

    context 'when the random number is 12' do
      it 'returns 7' do
        step_one = new_magic_seven.add_nine(random_number)
        step_two = new_magic_seven.multiply_by_two(step_one)
        step_three = new_magic_seven.subtract_four(step_two)
        step_four = new_magic_seven.divide_by_two(step_three)
        result = new_magic_seven.subtract_random_number(step_four)
