...
describe Cat do
  ...
  subject(:wack) { described_class.new('Wack', 'Ginger', 'Orange') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'When cat shares the same method name to other subclasses' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(wack).to_not be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(wack).to be_hiding
  end
end
