...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_matches) { double('number_matches', value: 3) }
      let(:guess_number) { 3 }
      subject(:game_end) { described_class.new(0, 9, number_matches, guess_number) }
    ...
    context 'when guess and random_number are not equal' do
      let(:goal_number) { double('goal_number', value: 3) }
      let(:guess_number) { 4 }
      subject(:game_not_over) { described_class.new(0, 9, goal_number, guess_number) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        min_value = low_guess_game.min
        expect(min_value).to eq(5)
      end

      it 'does not update max' do
        max_value = low_guess_game.max
        expect(max_value).to eq(9)
      end
    end
    ...
    context 'when the guess is more than the answer' do
      ...
      it 'does not update min' do
        min_value = high_guess_game.min
        expect(min_value).to eq(0)
      end

      it 'updates max to 8' do
        max_value = high_guess_game.max
        expect(max_value).to eq(8)
      end
    end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        min_value = guess_game.min
        expect(min_value).to eq(8)
      end

      it 'does not update max' do
        max_value = guess_game.max
        expect(max_value).to eq(8)
      end
    end
  end
