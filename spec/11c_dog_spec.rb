...
describe Dog do
  ...
  subject(:rush) { described_class.new('Rush', 'Labrador', 'White') }

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'When Dog has the same method name to other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(rush).to_not be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(rush).to be_sleeping
  end
end
